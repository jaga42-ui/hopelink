    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans overflow-x-hidden relative selection:bg-teal-500 selection:text-white flex flex-col">
      ...
      <nav className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 py-6 w-full flex justify-between items-center">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          <Link to="/login" className="px-5 py-2.5 sm:px-6 sm:py-3 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white rounded-full font-bold text-xs sm:text-sm transition-all active:scale-95 shadow-md flex items-center gap-2">
            <FaSignInAlt /> Sign In
          </Link>
          <Link to="/register" className="hidden sm:flex px-6 py-3 bg-teal-600 hover:bg-teal-500 text-white rounded-full font-extrabold text-sm shadow-lg shadow-teal-900/30 transition-all active:scale-95 items-center gap-2">
            <FaUserPlus /> Sign Up
          </Link>
        </div>
      </nav>
      ...
      >
        ...
        <motion.div animate={floatAnimation(0)} className="hidden lg:flex absolute top-32 left-10 items-center gap-4 bg-slate-900 border border-slate-800 px-6 py-4 rounded-3xl shadow-xl">
          ...
          <div className="text-left">
            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Urgent Need</p>
            <p className="text-sm font-semibold text-white">"Need O- Blood at City Hospital"</p>
          </div>
        </motion.div>

        <motion.div animate={floatAnimation(1.5)} className="hidden lg:flex absolute top-60 right-10 items-center gap-4 bg-slate-900 border border-slate-800 px-6 py-4 rounded-3xl shadow-xl">
          ...
          <div className="text-left">
            <p className="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Help Offered</p>
            <p className="text-sm font-semibold text-white">"Fresh groceries available to share"</p>
          </div>
        </motion.div>

        <motion.h1 variants={itemVariants} className="text-5xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] mb-6 text-white">
          Community is more than a place.<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-emerald-400 italic">It's how we care for each other.</span>
        </motion.h1>
        
        <motion.p variants={itemVariants} className="max-w-2xl text-slate-400 text-base sm:text-lg font-medium mb-10 leading-relaxed px-4">
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto z-20 relative px-4">
          <Link to="/register" className="w-full sm:w-auto px-10 py-4 bg-teal-600 hover:bg-teal-500 text-white rounded-full font-extrabold text-sm transition-all shadow-lg shadow-teal-900/40 active:scale-95 flex items-center justify-center gap-2">
            <FaHandsHelping className="text-lg" /> I Want to Help
          </Link>
          <Link to="/login" className="w-full sm:w-auto px-10 py-4 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white rounded-full font-bold text-sm transition-all shadow-md active:scale-95 flex items-center justify-center gap-2">
      ...
      <div className="relative z-20 max-w-6xl w-full mx-auto px-4 sm:px-6 -mt-10 sm:-mt-16 mb-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          <div className="bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-[2rem] relative overflow-hidden transition-all hover:bg-slate-800 shadow-xl">
            <h4 className="text-3xl sm:text-4xl font-black text-white mb-2">3 Lives</h4>
            <p className="text-slate-400 text-xs font-semibold leading-relaxed">The exact number of people you can save with a single donation of blood.</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-[2rem] relative overflow-hidden transition-all hover:bg-slate-800 shadow-xl">
            <h4 className="text-3xl sm:text-4xl font-black text-white mb-2">Minutes</h4>
            <p className="text-slate-400 text-xs font-semibold leading-relaxed">The time it takes for an emergency alert to reach people who can actually help.</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-[2rem] relative overflow-hidden transition-all hover:bg-slate-800 shadow-xl">
            <h4 className="text-3xl sm:text-4xl font-black text-white mb-2">Countless</h4>
            <p className="text-slate-400 text-xs font-semibold leading-relaxed">Moments of relief created when a community decides to stand together.</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 w-full bg-slate-900 border-y border-slate-800 py-24 shadow-inner">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="text-sm font-black tracking-[0.3em] uppercase text-teal-500 mb-6">Our Mission</h2>
      ...
      <div className="relative z-10 max-w-7xl w-full mx-auto px-4 sm:px-6 py-24">
        <div className="text-center mb-16">
          <h2 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 text-white">How people help people.</h2>
          <p className="text-slate-400 font-semibold tracking-wider uppercase text-sm">Simple, safe, and entirely free.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          ...
          >
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-3 text-white">Real-Time Alerts</h3>
          ...
          >
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-3 text-white">Share What You Have</h3>
            <p className="text-slate-400 text-sm leading-relaxed font-medium">
          ...
          >
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-3 text-white">Safe & Verified</h3>
            <p className="text-slate-400 text-sm leading-relaxed font-medium">
      ...
      <div className="relative z-10 w-full mt-auto">
        <div className="bg-slate-900 border-t border-slate-800 py-24 text-center px-4 shadow-inner">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/register" className="px-10 py-4 bg-teal-600 hover:bg-teal-500 text-white rounded-full font-extrabold text-sm shadow-lg shadow-teal-900/40 hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-2">
              <FaUserPlus /> Sign Up Now
            </Link>
            <Link to="/login" className="px-10 py-4 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white rounded-full font-bold text-sm transition-all shadow-md active:scale-95 flex items-center justify-center gap-2">
              <FaSignInAlt /> Sign In
            </Link>
          </div>
        </div>
